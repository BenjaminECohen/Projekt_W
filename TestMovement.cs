...
public class TestMovement : MonoBehaviour
{
    ...
    private void wallRunSafetyLock()
    {
        if (isGrounded && onWall)
        {
            wallSafetyLock = true;
        }
        if (wallSafetyLock && !onWall)
        {
            wallSafetyLock = false;
        }
    ...
    {
        if (!wallSafetyLock && isRunning && onWall && !wallRunning && !isGrounded)
        {
    ...
    {
        if (Input.GetKeyDown(KeyCode.LeftShift) && wallRunning)
        {
